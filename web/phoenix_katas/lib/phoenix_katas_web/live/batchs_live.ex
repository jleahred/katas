defmodule PhoenixKatasWeb.BatchsLive do
  ...
  def handle_event("del-" <> file, _, socket) do
    File.rm!("#{@batchs_path}/#{file}")
    {:noreply, update_files(socket)}
  end

  # defp get_batchs_fake() do
  #   [
  ...
  defp get_batchs() do
    ...
    |> Enum.sort_by(fn ff -> Map.get(ff, :folder) end, &<=/2)
  end
  ...
  defp get_file_and_folders_rec(path, acc) do
    ...
    current = %{
      files: ff.files |> Enum.sort_by(fn bi -> Map.get(bi, :date_time) end, &>=/2),
      folder: path
    }
