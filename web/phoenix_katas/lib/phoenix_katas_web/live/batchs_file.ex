defmodule PhoenixKatasWeb.BatchsFileLive do
  ...
  def render(assigns) do
    # <pre>
    # <%= inspect(File.stat!("./local/batchs/#{assigns.file_name}").mtime, pretty: true)%>
    # </pre>
    # <pre>
    ...
    ~L"""
      <div class="container">
      <h3>Batchs File <code><%=@file_name%></code></h3>
      checked <%= Time.to_string(@date_time) %><p>
      last updated file <%= Time.to_string(@last_updated) %>

        <pre style="width: 100%; height: 65vh; overflow-x: scroll;">
  ...
  def mount(session, socket) do
    ...
    socket = assign(socket, file_name: Map.get(session.par, "file_name"))
  ...
  defp get_lat_updated(file_name) do
    File.stat!("#{@batchs_path}/#{file_name}").mtime
