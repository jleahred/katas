...
void print_batch(const std::string &label,
                 const chi::proc::OutputBatch &batch) {
...
void run_self(const std::string_view name) {
  ...
  for (int mut_index = 0; mut_index < 10; ++mut_index) {
    std::cout << "I am " << name << " (" << mut_index + 1 << "/10)"
              << std::endl;
...
int main(const int argc, char *argv[]) {
  if (argc > 1 && std::string_view(argv[1]) == "--self") {
    const auto name =
        argc > 2 ? std::string_view(argv[2]) : std::string_view{"anonymous"};
    run_self(name);
    return 0;
  }
  ...
  struct Runner {
    std::string label;
    chi::proc::Process process;
  };

  Runner mut_first{"one", {}};
  Runner mut_second{"second", {}};

  const auto first_pid =
      mut_first.process.run({self_path, "--self", mut_first.label});
  const auto second_pid =
      mut_second.process.run({self_path, "--self", mut_second.label});
