...
Argv::Argv(std::vector<std::string> args)
    : mut_args(std::move(args))
{
    rebuild();
}
...
int Argv::argc() const
{
    return static_cast<int>(mut_args.size());
}

char** Argv::argv()
{
    return mut_argv.data();
}

char* const* Argv::argv() const
{
    return mut_argv.data();
}

const std::vector<std::string>& Argv::args() const
{
    return mut_args;
}
...
{
    mut_argv.clear();
    const auto argv_count = mut_args.size();
    mut_argv.reserve(argv_count + 1U);
    for (const auto& part : mut_args) {
        mut_argv.push_back(const_cast<char*>(part.c_str()));
    }
    mut_argv.push_back(nullptr);
}
...
{
#ifdef _WIN32
    // On Windows there's no softer signal, so force termination like request_stop
    return request_stop();
#else
    if (mut_pid == 0) {
        return false;
    ...
    const auto result = ::kill(mut_pid, SIGKILL);
    if (result != 0) {
        return false;
    }

    int        mut_status  = 0;
    const auto wait_result = waitpid(mut_pid, &mut_status, 0);
    if (wait_result == mut_pid) {
        read_pipe(mut_stdout_fd, mut_stdout_buffer, mut_stdout_pending);
        read_pipe(mut_stderr_fd, mut_stderr_buffer, mut_stderr_pending);
        mut_pid = 0;
...
bool Process::write(const std::string_view data)
{
    ...
    DWORD      mut_written = 0;
    const auto ok          = WriteFile(mut_stdin_write, data.data(), static_cast<DWORD>(data.size()),
                 &mut_written, nullptr);
...
bool Process::spawn_process(const std::vector<std::string>& command)
{
    ...
    if (!CreatePipe(&mut_stdout_read_local, &mut_stdout_write_local, &mut_sa,
            0)) {
        return false;
    }
    ...
    if (!CreatePipe(&mut_stderr_read_local, &mut_stderr_write_local, &mut_sa,
            0)) {
        CloseHandle(mut_stdout_read_local);
        CloseHandle(mut_stdout_write_local);
        return false;
    ...
    const auto created = CreateProcessA(
        nullptr, const_cast<char*>(command_line.c_str()), nullptr, nullptr, TRUE,
        0, nullptr, nullptr, &mut_startup_info, &mut_process_details);
...
void Process::read_pipe(HANDLE pipe_handle, std::string& buffer,
    std::vector<std::string>& lines)
{
    ...
    const auto            read_result    = ReadFile(pipe_handle, mut_chunk.data(),
                      static_cast<DWORD>(mut_chunk.size()), &mut_read_bytes, nullptr);
    ...
    for (auto mut_newline_pos = buffer.find('\n');
        mut_newline_pos != std::string::npos;
        mut_newline_pos = buffer.find('\n')) {
...
void Process::read_pipe(const int fd, std::string& buffer,
    std::vector<std::string>& lines)
{
    std::array<char, 512> mut_chunk{};
    while (true) {
