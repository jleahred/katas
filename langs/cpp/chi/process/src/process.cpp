...
bool Process::write(const std::string_view data) {
  ...
  DWORD mut_written = 0;
  const auto ok =
      WriteFile(mut_stdin_write, data.data(), static_cast<DWORD>(data.size()),
                &mut_written, nullptr);
...
void Process::read_pipe(HANDLE pipe_handle, std::string &buffer,
  ...
  const auto peek_result =
      PeekNamedPipe(pipe_handle, nullptr, 0, nullptr, &mut_available, nullptr);
  ...
  const auto read_result =
      ReadFile(pipe_handle, mut_chunk.data(),
...
bool Process::spawn_process(const std::vector<std::string> &command) {
  ...
  if (!make_non_blocking(mut_stdout_pipe[0]) ||
      !make_non_blocking(mut_stderr_pipe[0])) {
    close(mut_stdout_pipe[0]);
    close(mut_stderr_pipe[0]);
    close(mut_stdin_pipe[1]);
    return false;
