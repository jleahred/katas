...
bool read_uint16(const std::vector<uint8_t>& buffer, size_t& offset, uint16_t& value) {
...
bool read_uint32(const std::vector<uint8_t>& buffer, size_t& offset, uint32_t& value) {
...
bool read_uint64(const std::vector<uint8_t>& buffer, size_t& offset, uint64_t& value) {
...
Kvr::Kvr(const std::string& db_path)
    ...
    const auto pragma_result = sqlite3_exec(mut_db_, "PRAGMA foreign_keys = ON;", nullptr, nullptr, nullptr);
...
bool Kvr::ensure_bucket(const std::string& bucket_name) {
    ...
    const auto create_id_index_sql = "CREATE UNIQUE INDEX IF NOT EXISTS \"" + id_index_name + "\" "
        "ON \"" + table + "\" (id);";
    ...
    const auto create_index_sql = "CREATE UNIQUE INDEX IF NOT EXISTS \"" + index_name + "\" "
        "ON \"" + table + "\" (key, deleted_at);";
    if (!execute(create_index_sql)) {
        return false;
    }

    const auto active_index_name = table + "_key_active_idx";
    const auto create_active_index_sql = "CREATE UNIQUE INDEX IF NOT EXISTS \"" + active_index_name + "\" "
        "ON \"" + table + "\" (key, deleted_at) WHERE deleted_at = 0;";
    if (!execute(create_active_index_sql)) {
        return false;
    }

    const auto active_paging_index_name = table + "_key_active_id_idx";
    const auto create_active_paging_index_sql = "CREATE INDEX IF NOT EXISTS \"" + active_paging_index_name + "\" "
        "ON \"" + table + "\" (key, id) WHERE deleted_at = 0;";
...
bool Kvr::set(const std::string& bucket_name, const std::string& key, const std::string& value) {
    const std::lock_guard<std::recursive_mutex> lock(mut_db_mutex_);
    ...
    const auto update_prepare = sqlite3_prepare_v2(mut_db_, update_sql.c_str(), -1, &mut_update_stmt, nullptr);
    ...
    const auto insert_sql = "INSERT INTO \"" + table + "\" (created_at, deleted_at, key, value) "
        "VALUES (strftime('%s','now'), 0, ?, ?);";
    sqlite3_stmt* mut_insert_stmt = nullptr;
    const auto insert_prepare = sqlite3_prepare_v2(mut_db_, insert_sql.c_str(), -1, &mut_insert_stmt, nullptr);
...
bool Kvr::del(const std::string& bucket_name, const std::string& key, const std::string& value) {
    const std::lock_guard<std::recursive_mutex> lock(mut_db_mutex_);
    if (!ensure_bucket(bucket_name)) {
        return false;
    }
    ...
    const auto sql = "UPDATE \"" + table + "\" "
        "SET deleted_at = strftime('%s','now') "
        "WHERE key = ? AND value = ? AND deleted_at = 0;";

    sqlite3_stmt* mut_stmt = nullptr;
    const auto prepare_result = sqlite3_prepare_v2(mut_db_, sql.c_str(), -1, &mut_stmt, nullptr);
    ...
    sqlite3_bind_text(mut_stmt, 2, value.c_str(), -1, SQLITE_TRANSIENT);
...
std::optional<std::string> Kvr::get(const std::string& bucket_name, const std::string& key) {
    const std::lock_guard<std::recursive_mutex> lock(mut_db_mutex_);
    if (!ensure_bucket(bucket_name)) {
        return std::nullopt;
    }
...
std::vector<std::string> Kvr::list_buckets() {
    ...
    const auto sql = "SELECT name FROM sqlite_master "
        "WHERE type = 'table' AND name NOT LIKE 'sqlite_%' ORDER BY name ASC;";
    sqlite3_stmt* mut_stmt = nullptr;
    const auto prepare_result = sqlite3_prepare_v2(mut_db_, sql, -1, &mut_stmt, nullptr);
    if (prepare_result != SQLITE_OK) {
    ...
    while (sqlite3_step(mut_stmt) == SQLITE_ROW) {
        const auto mut_name = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 0));
        if (mut_name != nullptr) {
...
Kvr::KeyListResult Kvr::list_active_keys(const std::string& bucket_name, const std::string& key_prefix,
    ...
    const auto sql = "SELECT key, id FROM \"" + table + "\" "
        "WHERE deleted_at = 0 AND key LIKE ? AND (key > ? OR (key = ? AND id > ?)) "
    ...
    while (sqlite3_step(mut_stmt) == SQLITE_ROW) {
        const auto mut_key = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 0));
...
void Kvr::add_poll_server(const std::string& bucket_name, TcpSide tcp_side,
    const std::string& host, uint16_t port, uint32_t poll_max_reply_count) {
    const std::lock_guard<std::mutex> lock(mut_poll_mutex_);
    auto* mut_endpoint = find_poll_endpoint(tcp_side, host, port);
    if (mut_endpoint == nullptr) {
        mut_poll_endpoints_.push_back(PollEndpoint{tcp_side, host, port, {}, {}, false});
        mut_endpoint = &mut_poll_endpoints_.back();
    }

    add_poll_server_bucket(*mut_endpoint, bucket_name, poll_max_reply_count);
    if (mut_started_) {
...
void Kvr::add_poll_client(const std::string& bucket_name, TcpSide tcp_side,
    const std::string& host, uint16_t port, std::chrono::milliseconds poll_interval) {
    const std::lock_guard<std::mutex> lock(mut_poll_mutex_);
    auto* mut_endpoint = find_poll_endpoint(tcp_side, host, port);
    if (mut_endpoint == nullptr) {
        mut_poll_endpoints_.push_back(PollEndpoint{tcp_side, host, port, {}, {}, false});
...
bool Kvr::start() {
    ...
    mut_work_guard_ = std::make_unique<asio::executor_work_guard<asio::io_context::executor_type>>(
    ...
    mut_io_thread_ = std::thread([this]() {
        mut_io_context_->run();
    });
...
const std::string& Kvr::last_error() const {
...
bool Kvr::execute(const std::string& sql) {
    ...
    const auto result = sqlite3_exec(mut_db_, sql.c_str(), nullptr, nullptr, &mut_error);
    if (result != SQLITE_OK) {
...
void Kvr::add_poll_server_bucket(PollEndpoint& endpoint, const std::string& bucket_name,
    uint32_t poll_max_reply_count) {
    const auto it = std::find_if(endpoint.mut_poll_server_buckets.begin(),
    ...
    if (it == endpoint.mut_poll_server_buckets.end()) {
        endpoint.mut_poll_server_buckets.push_back(PollServerBucketConfig{bucket_name, poll_max_reply_count});
    } else {
        it->poll_max_reply_count = poll_max_reply_count;
    }
}

void Kvr::add_poll_client_bucket(PollEndpoint& endpoint, const std::string& bucket_name,
    std::chrono::milliseconds poll_interval) {
    const auto it = std::find_if(endpoint.mut_poll_client_buckets.begin(),
    ...
    if (it == endpoint.mut_poll_client_buckets.end()) {
        endpoint.mut_poll_client_buckets.push_back(PollClientBucketConfig{bucket_name, poll_interval});
    } else {
        it->poll_interval = poll_interval;
    }
}

Kvr::PollEndpoint* Kvr::find_poll_endpoint(TcpSide tcp_side, const std::string& host, uint16_t port) {
    for (auto& endpoint : mut_poll_endpoints_) {
        if (endpoint.tcp_side == tcp_side && endpoint.host == host && endpoint.port == port) {
            return &endpoint;
        }
    }
    return nullptr;
...
bool Kvr::start_acceptor(PollEndpoint& endpoint) {
    ...
    auto mut_acceptor = std::make_unique<asio::ip::tcp::acceptor>(*mut_io_context_);
    ...
    mut_acceptor->set_option(asio::ip::tcp::acceptor::reuse_address(true), mut_ec);
    if (mut_ec) {
    ...
    *mut_accept_loop = [this, mut_acceptor_ptr, poll_server = endpoint.mut_poll_server_buckets,
        poll_client = endpoint.mut_poll_client_buckets, mut_accept_loop]() {
        auto mut_socket = std::make_shared<asio::ip::tcp::socket>(*mut_io_context_);
        mut_acceptor_ptr->async_accept(*mut_socket,
            [this, poll_server, poll_client, mut_socket, mut_accept_loop](const asio::error_code& ec) {
            if (!ec) {
                ...
                for (const auto& config : poll_client) {
                    ...
                    state.interval = config.poll_interval;
...
bool Kvr::start_connect(PollEndpoint& endpoint) {
    ...
    auto mut_socket = std::make_shared<asio::ip::tcp::socket>(*mut_io_context_);
    auto mut_resolver = std::make_shared<asio::ip::tcp::resolver>(*mut_io_context_);
    const auto port_string = std::to_string(endpoint.port);
    mut_resolver->async_resolve(endpoint.host, port_string,
        [this, mut_socket, mut_resolver, poll_server = endpoint.mut_poll_server_buckets,
            poll_client = endpoint.mut_poll_client_buckets, endpoint_ptr = &endpoint](const asio::error_code& ec,
            ...
            asio::async_connect(*mut_socket, results,
                [this, mut_socket, poll_server, poll_client, endpoint_ptr](const asio::error_code& connect_ec,
...
void Kvr::start_polling(const std::shared_ptr<PollConnection>& connection) {
    ...
    for (size_t index = 0; index < connection->mut_poll_client_states.size(); ++index) {
...
void Kvr::schedule_poll(const std::shared_ptr<PollConnection>& connection, size_t index) {
    ...
    state.mut_timer->expires_after(state.interval);
    state.mut_timer->async_wait([this, connection, index](const asio::error_code& ec) {
        if (ec || !mut_started_) {
            return;
        }

        if (index < connection->mut_poll_client_states.size()) {
            const auto& bucket_name = connection->mut_poll_client_states[index].name;
            send_poll(connection, bucket_name);
        }
...
void Kvr::start_reading(const std::shared_ptr<PollConnection>& connection) {
    ...
    asio::async_read(*connection->mut_socket, asio::buffer(connection->mut_header),
        [this, connection](const asio::error_code& ec, std::size_t) {
            if (ec) {
                return;
            }

            const uint32_t length = (static_cast<uint32_t>(connection->mut_header[0]) << 24)
            ...
            asio::async_read(*connection->mut_socket, asio::buffer(connection->mut_body),
...
void Kvr::handle_message(const std::shared_ptr<PollConnection>& connection) {
    ...
    if (type == k_message_poll) {
        ...
        const auto bucket_name = std::string(
            reinterpret_cast<const char*>(connection->mut_body.data() + offset), bucket_size);
        offset += bucket_size;
        ...
        if (has_first) {
            ...
            first_key.assign(
                reinterpret_cast<const char*>(connection->mut_body.data() + offset), key_size);
            offset += key_size;
            first_value.assign(
                reinterpret_cast<const char*>(connection->mut_body.data() + offset), value_size);
            offset += value_size;
        }
        ...
        const auto it = std::find_if(connection->mut_poll_server_buckets.begin(),
        ...
        const auto server_has_first = get_first_record(bucket_name, server_first_id,
            server_first_key, server_first_value);
        if (has_first) {
            if (!server_has_first
                || static_cast<int64_t>(first_id) != server_first_id
                || first_key != server_first_key
                || first_value != server_first_value) {
        ...
        std::vector<PollRecord> records;
        {
            ...
            const auto table = table_name(bucket_name);
            const auto sql = "SELECT id, created_at, deleted_at, key, value FROM \"" + table + "\" "
            ...
            sqlite3_bind_int(mut_stmt, 2, static_cast<int>(it->poll_max_reply_count));
            ...
            while (sqlite3_step(mut_stmt) == SQLITE_ROW) {
                ...
                const auto mut_key = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 3));
                const auto mut_value = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 4));
        ...
        for (const auto& record : records) {
            send_reply(connection, bucket_name, record.id, record.created_at, record.deleted_at,
    ...
    if (type == k_message_reply) {
        ...
        apply_reply(bucket_name, static_cast<int64_t>(id), static_cast<int64_t>(created_at),
            static_cast<int64_t>(deleted_at), key, value);
        return;
    }
...
void Kvr::send_poll(const std::shared_ptr<PollConnection>& connection, const std::string& bucket_name) {
    int64_t first_id = 0;
    std::string first_key;
    std::string first_value;
    const auto has_first = get_first_record(bucket_name, first_id, first_key, first_value);
    const auto last_id = get_last_id(bucket_name);
    std::vector<uint8_t> payload;
    payload.reserve(1 + 2 + bucket_name.size() + 1 + 8 + 4 + 4 + first_key.size() + first_value.size() + 8);
...
void Kvr::send_reply(const std::shared_ptr<PollConnection>& connection, const std::string& bucket_name,
    int64_t id, int64_t created_at, int64_t deleted_at, const std::string& key, const std::string& value) {
    std::vector<uint8_t> payload;
    payload.reserve(1 + 2 + bucket_name.size() + 8 + 8 + 8 + 4 + 4 + key.size() + value.size());
...
void Kvr::send_reset(const std::shared_ptr<PollConnection>& connection, const std::string& bucket_name) {
    std::vector<uint8_t> payload;
...
bool Kvr::get_first_record(const std::string& bucket_name, int64_t& id, std::string& key, std::string& value) {
    const std::lock_guard<std::recursive_mutex> lock(mut_db_mutex_);
    if (!ensure_bucket(bucket_name)) {
        return false;
    }
    ...
    const auto table = table_name(bucket_name);
    const auto sql = "SELECT id, key, value FROM \"" + table + "\" ORDER BY id ASC LIMIT 1;";
    sqlite3_stmt* mut_stmt = nullptr;
    const auto prepare_result = sqlite3_prepare_v2(mut_db_, sql.c_str(), -1, &mut_stmt, nullptr);
    if (prepare_result != SQLITE_OK) {
    ...
    const auto mut_key = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 1));
    const auto mut_value = reinterpret_cast<const char*>(sqlite3_column_text(mut_stmt, 2));
...
bool Kvr::apply_reply(const std::string& bucket_name, int64_t id, int64_t created_at, int64_t deleted_at,
    ...
    const auto delete_prepare = sqlite3_prepare_v2(mut_db_, delete_sql.c_str(), -1, &mut_delete_stmt, nullptr);
    ...
    const auto sql = "INSERT INTO \"" + table + "\" (id, created_at, deleted_at, key, value) "
        "VALUES (?, ?, ?, ?, ?) "
