...
- `bool del(const std::string& bucket_name, const std::string& key, const std::string& value)`: marca como borrada la clave; esto son dos pasos, se marca el registro "activo" como borrado, y se añade un nuevo registro marcado tambień como borrado con el value dado en esta función. Este registro nuevo se crea evitar problemas en sincronización. La lógica de este value la define la aplicación.
...
- `bool start()`: inicia TCP/polling.
- `void stop()`: detiene TCP/polling.
