...
bool read_u32(std::istream& is, std::uint32_t& value)
{
    ...
    value = (std::uint32_t{ static_cast<std::uint8_t>(mut_buf[0]) }) | (std::uint32_t{ static_cast<std::uint8_t>(mut_buf[1]) } << 8) | (std::uint32_t{ static_cast<std::uint8_t>(mut_buf[2]) } << 16) | (std::uint32_t{ static_cast<std::uint8_t>(mut_buf[3]) } << 24);
    ...
    std::uint64_t&                                  header_end_out)
{
    ...
    if (!read_u32(mut_in, mut_sig_len) || mut_sig_len == 0 || mut_sig_len > kMaxSignatureSize) {
        return false;
    }
    ...
    while (true) {
        ...
        if (static_cast<std::byte>(mut_start[0]) != kStartA || static_cast<std::byte>(mut_start[1]) != kStartB) {
            return false;
        }
...
void QueueWriter::reset()
{
    ...
    file_.close();
    const auto parent = queue_path_.parent_path();
    if (!parent.empty()) {
        std::error_code mut_ec;
        std::filesystem::create_directories(parent, mut_ec);
        if (mut_ec) {
            throw std::runtime_error("Unable to create queue directory: " + mut_ec.message());
        }
    }
...
QueueReader::QueueReader(std::filesystem::path queue_path)
    : QueueReader(std::move(queue_path), std::filesystem::path{})
{
}

QueueReader::QueueReader(std::filesystem::path queue_path, std::filesystem::path cursor_path)
    : queue_path_(std::move(queue_path))
...
void QueueReader::loadCursor()
{
    ...
    std::string              mut_line;
    while (std::getline(mut_in, mut_line)) {
        if (!mut_line.empty() && mut_line.back() == '\r') {
            mut_line.pop_back();
        }
        mut_lines.emplace_back(std::move(mut_line));
    }

    if (mut_lines.size() >= 3) {
        if (mut_lines[0] != queue_path_.string()) {
            cursor_signature_.clear();
            cursor_offset_ = 0;
            return;
        }
        cursor_signature_ = mut_lines[1];
        std::istringstream mut_iss(mut_lines[2]);
        mut_iss >> cursor_offset_;
        return;
    }

    if (!mut_lines.empty()) {
        cursor_signature_ = mut_lines[0];
    }
    if (mut_lines.size() >= 2) {
        std::istringstream mut_iss(mut_lines[1]);
        mut_iss >> cursor_offset_;
    }
}
...
void QueueReader::storeCursor()
{
    ...
    if (std::filesystem::exists(cursor_path_)) {
        ...
        if (mut_ec) {
            throw std::runtime_error("Unable to rotate cursor to .prev: " + mut_ec.message());
        }
    }
...
ReadResult QueueReader::read(const std::size_t max_records)
{
    ...
    while (result.size() < max_records) {
        ...
        result.add_message(std::move(mut_payload));
    }
