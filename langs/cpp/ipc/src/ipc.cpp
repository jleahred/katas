...
// Scan file to find the last valid offset. Returns false if the signature is invalid.
...
void QueueReader::storeCursor() {
    std::ofstream mut_out(cursor_path_, std::ios::binary | std::ios::trunc);
    if (!mut_out) {
        throw std::runtime_error("Unable to write cursor");
    }
