...
fn make_a_division_for_ever() {
    loop {
        ...
        //  reducing boilerplate with let it crash (expect)        
...
fn make_a_division_for_ever() {
    loop {
        println!("Enter divisor...");
        //  reducing boilerplate with let it crash (expect)        
        let divisor = std::io::stdin()
                    .read_line()
                    .ok().expect("error reading line, I' think I will die")
                    .trim()
                    .parse::<u32>().expect("I couldn't parse the line, I'm ready to die");
