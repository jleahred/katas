defmodule Wui7Web.UserAuth do
  ...
  alias Wui7.Accounts
  alias Wui7.Accounts.Scope
  alias Phoenix.LiveView

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_wui7_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      Wui7Web.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def on_mount(:ensure_authenticated_scope, _params, session, socket) do
    socket = assign_current_scope(socket, session)

    if scope_authenticated?(socket.assigns) do
      {:cont, socket}
    else
      {:halt,
       socket
  ...
  def on_mount(:redirect_if_scope_is_authenticated, _params, session, socket) do
    socket = assign_current_scope(socket, session)
  ...
  defp scope_authenticated?(assigns) do
