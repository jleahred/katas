defmodule MsgParse do
  ...
  * All status has the field msg_inf of type MsgInfo (parsed status)
  ...
  ###  Examples
    ...
    iex> stpars = MsgParse.add_char(%MsgParse.StFullMessage{}, ?8)
    %MsgParse.StPartTag{chunk: "8", msg_inf: %MsgParse.MsgInfo{body_length: 1, check_sum: 56, map_msg: %{}}}
"""

  defp add_error(error_list, new_error_desc)  do
    cond  do
  ...
  def add_char(%StFullMessage{ msg_inf: msg_inf }, 1)  do
    %StFullMessage{
      msg_inf: %MsgInfo
               { msg_inf
               | errors: add_error(msg_inf.errors,
                  "Invalid SOH after full message  recieved #{msg_inf.orig_msg}")
              }
    }
  end

  def add_char(%StFullMessage{ msg_inf: _msg_inf }, ch)  do
      add_char(%StPartTag{}, ch)
  end

  def add_char(%StPartTag{msg_inf: msg_inf, chunk: chunk}, ?=)  do
  ...
  def add_char(%StPartTag{msg_inf: msg_inf, chunk: chunk}, 1)  do
      %StPartTag {
        msg_inf: %MsgInfo
                     { msg_inf
                     | errors:    add_error(msg_inf.errors,
  ...
  def add_char(%StPartTag{msg_inf: msg_inf, chunk: chunk}, ch)  do
      %StPartTag {
        msg_inf: %MsgInfo
                      { msg_inf |
  ...
  def add_char(%StPartVal{msg_inf: msg_inf, tag: 10, chunk: chunk}, 1)  do
      %StFullMessage {
  ...
  def add_char(%StPartVal{msg_inf: msg_inf, tag: tag, chunk: chunk}, 1)  do
    %StPartTag {
      msg_inf: %MsgInfo
                  { msg_inf
  ...
  def add_char(%StPartVal{msg_inf: msg_inf, tag: tag, chunk: chunk}, ch)  do
    %StPartVal {
      msg_inf: %MsgInfo { msg_inf
                   | body_length: msg_inf.body_length + if(tag==8 or tag==9 or tag==10, do: 0, else: 1),
  ...
  def test_parse_string do
      ...
      msg_list |> Enum.reduce(%StFullMessage{}, &(add_char(&2, &1)))
  end
  ...
  def test_parse_string_perf do
      ...
      funt = fn -> Enum.each(1..num_msg,
          fn(_) -> msg_list |> Enum.reduce(%StFullMessage{}, &(add_char(&2, &1))) end)
        end
